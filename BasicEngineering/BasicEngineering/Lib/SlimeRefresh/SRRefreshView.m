...
@property (nonatomic, assign)   BOOL   isEnadleRefreshFooter;
@end
...
{
    CGPoint p = _scrollView.contentOffset;
    CGFloat pointY=_scrollView.contentSize.height+_scrollView.contentInset.bottom-_scrollView.frame.size.height;
    if(p.y>(20-self.upInset+(pointY>0?(pointY+self.upInset):0))){
        _isEnadleRefreshFooter=YES;
    }else{
        _isEnadleRefreshFooter=NO;
    }
...
{
    if(_isEnadleRefreshFooter){
        if([self.delegate respondsToSelector:@selector(slimeRefreshstartFooterFresh)]){
            [self.delegate slimeRefreshstartFooterFresh];
        }
    }
    if (_broken) {
        if (self.loading) {
